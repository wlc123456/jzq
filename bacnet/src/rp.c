    										   ...
    										   BACNET_READ_PROPERTY_DATA * rpdata)
{
    ...
    if (rpdata != NULL) {
        ...
        debug_printf("%s,%d,object_type = %d,object_instance = %d\n",__FUNCTION__,__LINE__,rpdata->object_type,rpdata->object_instance);
        /* Tag 1: Property ID */
	...
	debug_printf("%s,%d,property = %d\n",__FUNCTION__,__LINE__,rpdata->object_property);
		...
		debug_printf("%s,%d,array_index = %d\n",__FUNCTION__,__LINE__,rpdata->array_index);
            } else {
                rpdata->error_code = ERROR_CODE_REJECT_INVALID_TAG;
                return BACNET_STATUS_REJECT;
            }
    												...
    												BACNET_READ_PROPERTY_DATA * rpdata)
{
	...
	debug_printf("%d,%d\n",apdu_len,len);
	debug_printf("%s,%d,rpdata->application_data_len = %d\n",__FUNCTION__,__LINE__,rpdata->application_data_len);
