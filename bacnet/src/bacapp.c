...
volatile extern uint32_t  Object_Num ;
volatile extern uint16_t type_temp ;
volatile extern uint32_t instance_temp ;
...
#if defined (BACAPP_UNSIGNED)
            case BACNET_APPLICATION_TAG_UNSIGNED_INT:
                len = decode_unsigned(&apdu[0], len_value_type,&value->type.Unsigned_Int);
		Object_Num = value->type.Unsigned_Int;
		debug_printf("%s,%d,value->type.Unsigned_Int = %u\n",__FUNCTION__,__LINE__,value->type.Unsigned_Int);
                break;
#endif
#if defined (BACAPP_SIGNED)
            case BACNET_APPLICATION_TAG_SIGNED_INT:
		    ...
		    type_temp =  value->type.Object_Id.type;
		    instance_temp = value->type.Object_Id.instance;
                }
                break;
#endif
            default:
     												   ...
     												   BACNET_APPLICATION_DATA_VALUE * value)
{
	...
	debug_printf("%s,%d\n",__FUNCTION__,__LINE__);
        ...
        tag_len = decode_tag_number_and_value(&apdu[0], &tag_number,&len_value_type);
        if (tag_len) 
	{
            len += tag_len;
            value->tag = tag_number;
	    debug_printf("%s,%d\n",__FUNCTION__,__LINE__);
            decode_len = bacapp_decode_data(&apdu[len], tag_number, len_value_type,value);
 ...
 */
bool bacapp_print_value(FILE * stream,BACNET_OBJECT_PROPERTY_VALUE * object_value)
{
    char *str;
    bool retval = false;
    size_t str_len = 32;
