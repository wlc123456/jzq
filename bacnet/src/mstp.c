...
{       /* number of bytes of data (up to 501) */
    ...
    len = MSTP_Create_Frame((uint8_t *) & mstp_port->OutputBuffer[0],
             mstp_port->OutputBufferSize, frame_type, destination, source, 
             data,data_len);
...
void MSTP_Receive_Frame_FSM(volatile struct mstp_port_struct_t *mstp_port)
{
	...
	switch (mstp_port->receive_state) 
	{
		...
		{
                	printf_receive_data("Rx: %02X ",mstp_port->DataRegister);
                	/* Preamble1 */
		...
		else if (mstp_port->DataAvailable == true) 
		{
                	...
                	else if (mstp_port->Index == 5) 
			{
                   		...
                   		if (mstp_port->HeaderCRC != 0x55) 
				{
					...
					{
                           			 /* NoData */
                            			printf_receive_data("%s",mstptext_frame_type((unsigned)mstp_port->FrameType));
					  ...
					  else 
					  {
							...
							{
                                    				/* FrameTooLong */
                                    				printf_receive_error("MSTP: Rx Header: FrameTooLong %u\n",
            	...
            	if (mstp_port->SilenceTimer((void *) mstp_port) > Tframe_abort) 
		{
               		/* indicate that an error has occurred during the reception of a frame */
                	mstp_port->ReceivedInvalidFrame = true;
                	printf_receive_error("MSTP: Rx Data: SilenceTimer %ums > %dms\n",
                    					 (unsigned) mstp_port->SilenceTimer((void *) mstp_port),Tframe_abort);
                	/* wait for the start of the next frame. */
                	mstp_port->receive_state = MSTP_RECEIVE_STATE_IDLE;
            	}
    	...
    	if ((receive_state != MSTP_RECEIVE_STATE_IDLE) &&(mstp_port->receive_state == MSTP_RECEIVE_STATE_IDLE)) 
	{
        	printf_receive_data("\n");
        	fflush(stderr);
    	}
...
bool MSTP_Master_Node_FSM(volatile struct mstp_port_struct_t * mstp_port)
{
	...
	switch (mstp_port->master_state) 
	{
		...
		else 
		{
                	...
                	RS485_Send_Frame(mstp_port,(uint8_t *) & mstp_port->OutputBuffer[0],(uint16_t) length);
