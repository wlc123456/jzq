...
static uint8_t tsm_find_invokeID_index(uint8_t invokeID)
{
	...
	{
            //debug_printf("%s,%d,i = %d,TSM_List[%d].InvokeID = %u\n",__FUNCTION__,__LINE__,i,i,TSM_List[i].InvokeID );
            index = (uint8_t) i;
            break;
        }
    }
     //debug_printf("%s,%d,index = %u\n",__FUNCTION__,__LINE__,index);
    return index;
}
...
//寻找空闲的事物状态机，找到返回true，没找到返回false
...
uint8_t tsm_next_free_invokeID(void)
{
    ...
    {
        while (!found) 
	{ //寻找与Current_Invoke_ID相匹配的状态机下标，没有匹配的返回255
            index = tsm_find_invokeID_index(Current_Invoke_ID);
	    //debug_printf("%s,%d,index = %d\n",__FUNCTION__,__LINE__,index);
...
void tsm_timer_milliseconds(uint16_t milliseconds)
{
        ...
        if (TSM_List[i].state == TSM_STATE_AWAIT_CONFIRMATION) 
	{
                ...
                if (TSM_List[i].RetryCount < apdu_retries()) 
		{
		    ...
		    //debug_printf("%s,%d\n",__FUNCTION__,__LINE__);
                    datalink_send_pdu_bacnet_mstp(&TSM_List[i].dest,&TSM_List[i].npdu_data, &TSM_List[i].apdu[0],TSM_List[i].apdu_len);
                } 
		else 
		{
