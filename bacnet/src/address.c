...
static struct Address_Cache_Entry *address_remove_oldest(void)
{
    ...
    while (pMatch <= &Address_Cache[MAX_ADDRESS_CACHE - 1]) 
    {
        if ((pMatch->Flags & (BAC_ADDR_IN_USE | BAC_ADDR_BIND_REQ |BAC_ADDR_STATIC)) ==
...
static void address_file_init(const char *pFilename)
{
        ...
        while (fgets(line, (int) sizeof(line), pFile) != NULL) 
	{
            ...
            if (line[0] != ';') 
	    {
                if (sscanf(line, "%7ld %79s %5u %79s %4u", 
		     &device_id,&mac_string[0], &snet, &sadr_string[0],&max_apdu) == 5) 
		{
                    count = sscanf(mac_string, "%2x:%2x:%2x:%2x:%2x:%2x", 
					      &mac[0],&mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
		    //从address_cache中获取绑定设备mac地址及mac地址长度
                    src.mac_len = (uint8_t) count;
		    debug_printf("%s,%d,src.mac_len = %d\n",__FUNCTION__,__LINE__,src.mac_len);
                    for (index = 0; index < MAX_MAC_LEN; index++) 
		    {
                        src.mac[index] = (uint8_t) mac[index];
                    }
		    //从address_cache中获取绑定设备的网络号，网络地址长度及
		    //网络地址
                    src.net = (uint16_t) snet;
                    if (snet) 
		    {
                        count = sscanf(sadr_string, "%2x:%2x:%2x:%2x:%2x:%2x",
                            			  &mac[0], &mac[1], &mac[2], &mac[3], &mac[4],&mac[5]);
		    ...
		    //将address_cache中的设备信息复制到struct Address_Cache_Entry 结构体中保存
...
void address_set_device_TTL(uint32_t device_id,uint32_t TimeOut,bool StaticFlag)
{
    struct Address_Cache_Entry *pMatch;
...
/*
bool address_get_by_device(uint32_t device_id,unsigned *max_apdu,BACNET_ADDRESS * src)
{
    struct Address_Cache_Entry *pMatch;
    bool found = false;
...
bool address_get_by_device(uint32_t device_id,unsigned *max_apdu,BACNET_ADDRESS * src)
{
	struct address_entry *p_addr_entry;
    	bool found = false; /* return value */

    	p_addr_entry = Address_Table.first;
    	while (p_addr_entry != NULL) 
    	{
        	if (((p_addr_entry->Flags & BAC_ADDRESS_INUSE ) != 0) &&(p_addr_entry->device_id == device_id)) 
		{
			*src = p_addr_entry->address;
                	*max_apdu = p_addr_entry->max_apdu;
			debug_printf("%s,%d,max_apdu = %d\n",__FUNCTION__,__LINE__,*max_apdu);
...
/*************************************************************************
*  名字:  address_add
*  说明: 将address_cache文件中存储的设备相关信息保存到
*                 struct Address_Cache_Entry 结构体中
*  输入参数：device_id:设备ID
*					  max_apdu:最大应用协议单元的长度
*					  src:目的地址信息
*         
*  返回值: 无
*************************************************************************/
...
void address_add_binding(uint32_t device_id,unsigned max_apdu,BACNET_ADDRESS * src)
{
    struct Address_Cache_Entry *pMatch;
...
void address_table_add(uint32_t device_id,unsigned max_apdu,BACNET_ADDRESS * src)
{
        ...
        if ((pMatch->Flags & (BAC_ADDR_IN_USE | BAC_ADDR_RESERVED)) == 0) 
	{
	   	debug_printf("%s,%d,i = %d\n",__FUNCTION__,__LINE__,i);
    		...
    		pMatch->TimeToLive = BAC_ADDR_FOREVER;
    		debug_printf("%s,%d\n",__FUNCTION__,__LINE__);
    		debug_printf("device_id = %u,max_apdu = %u,flags = %u\n",pMatch->device_id ,pMatch->max_apdu,pMatch->Flags);
    		printf("src->mac_len = %u\n",pMatch->address.mac_len);
    		for(i = 0; i< MAX_MAC_LEN;i++)
    		{
			printf("%u:",pMatch->address.mac[i]);
   		 }
    		printf("\n");
    		printf("src->net = %u\n",pMatch->address.net);
    		printf("src->len = %u\n",pMatch->address.len);
...
void Send_Read_Property_Parameter_Init(uint32_t *Target_Device_Object_Instance,
					    							       BACNET_OBJECT_TYPE *Target_Object_Type, 
			                    							       uint32_t *Target_Object_Instance,
					    							       BACNET_PROPERTY_ID *Target_Object_Property, 
					    					 		       int32_t  *Target_Object_Index)
{
	*Target_Device_Object_Instance =  8;
	*Target_Object_Type = 8;
 	*Target_Object_Instance = 8;
  	
 	*Target_Object_Property = 8;
 	*Target_Object_Index = 8;
}
