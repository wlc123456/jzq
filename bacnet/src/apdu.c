...
static unconfirmed_function Unconfirmed_Function[MAX_BACNET_UNCONFIRMED_SERVICE];
...
/********************************************************************
* 函 数 名    :apdu_decode_confirmed_service_request
* 函数介绍:	
* 输入参数:	apdu:apdu协议数据单元的首地址
*                         	apdu_len : apdu协议数据单元的长度
*					service_data :对apdu协议数据单元的apci 部分的结构体描述
*					service_choice: 证实的服务类别
*					service_request:指向apdu 用户数据部分的首地址的指针
*					service_request_len:apdu协议数据单元的用户数据部分数据的长度
* 输出参数:		
* 返 回 值    :		
********************************************************************/
		...
		uint16_t * service_request_len)
{
	...
	service_data->segmented_response_accepted = (apdu[0] & BIT1) ? true : false; //分段复杂确认
	service_data->max_segs = decode_max_segs(apdu[1]);	//发送服务请求的设备所能接受的最大分段数量或者是保留值全是0
	service_data->max_resp = decode_max_apdu(apdu[1]);	//发送服务请求的设备所能接受的最大的报文长度
	service_data->invoke_id = apdu[2];		//证实服务和请求响应的联系
	len = 3;
	...
	*service_choice = apdu[len++];		//证实服务的类别 eg:readProperty  readProperyMultiple len == 4
	debug_printf("%s,%d,service_choice = %02x\n",__FUNCTION__,__LINE__,*service_choice);
	*service_request = &apdu[len];
	*service_request_len = apdu_len - len;
	 debug_printf("%s,%d,apdu_len = %d,len = %d,service_request_len = %d\n",__FUNCTION__,__LINE__,apdu_len,len,*service_request_len);
 ...
 *  apdu 协议数据单元的长度
 */
void apdu_handler(BACNET_ADDRESS * src,uint8_t * apdu, /* APDU data */uint16_t apdu_len)
{
	...
	if (apdu) 
	{
        	...
        	switch (apdu[0] & 0xF0) 
		{
                	...
                	len =  (int) apdu_decode_confirmed_service_request(&apdu[0],apdu_len, &service_data, 
														      &service_choice, &service_request,
                    												      &service_request_len);
                	if (apdu_confirmed_dcc_disabled(service_choice)) 
			{
                	...
                	if ((service_choice < MAX_BACNET_CONFIRMED_SERVICE) &&(Confirmed_Function[service_choice]))
                    		Confirmed_Function[service_choice] (service_request,service_request_len, src, &service_data);
                	else if (Unrecognized_Service_Handler)
                    		Unrecognized_Service_Handler(service_request,service_request_len, src, &service_data);
                		break;
                	...
                	if (service_choice < MAX_BACNET_UNCONFIRMED_SERVICE) 
			{
                    		if (Unconfirmed_Function[service_choice])
                        		Unconfirmed_Function[service_choice] (service_request,service_request_len, src);
                	}
                	break;
                ...
                switch (service_choice) 
		{
                        ...
                        if (Confirmed_ACK_Function[service_choice] != NULL) 
			{
			  ...
			  //service_request_len:apdu协议数据单元的用户数据部分数据的长度
			  //src  消息的源地址
			  //service_ack_data :相应信息的APCI控制部分的信息内容
