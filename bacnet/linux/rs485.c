...
static char *RS485_Port_Name = "/dev/ttymxc1";
...
void RS485_Check_UART_Data(volatile struct mstp_port_struct_t *mstp_port)
{
    	...
    	//debug_printf("%s,%d\n",__FUNCTION__,__LINE__);
    	SHARED_MSTP_DATA *poSharedData = (SHARED_MSTP_DATA *) mstp_port->UserData;
	//debug_printf("%s,%d,poSharedData = %d\n",__FUNCTION__,__LINE__,poSharedData);
    	if (!poSharedData) 
	{
...
void RS485_Initialize(void)
{
    ...
    debug_printf("%s,%d,RS485_Handle = %d\n",__FUNCTION__,__LINE__,RS485_Handle);
#if 0
    /* non blocking for the read */
    fcntl(RS485_Handle, F_SETFL, FNDELAY);
#else
    /* efficient blocking for the read */
   //test  = fcntl(RS485_Handle, F_GETFL, 0);
   //debug_printf("%s,%d,test = %d\n",__FUNCTION__,__LINE__,test);
   //???¡ä¡ä??¨²¦Ì?¡Á¡ä¨¬??a¡Á¨¨¨¨?¡Á¡ä¨¬?¡ê?¨®?¨®¨²¦Ì¨¨¡äy¡ä??¨²¨ºy?Y¦Ì??¨¢¨¨??¡ê
   test  = fcntl(RS485_Handle, F_SETFL, 0);
   debug_printf("%s,%d,test = %d\n",__FUNCTION__,__LINE__,test);
#endif
