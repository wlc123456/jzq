...
//#define DEBUG
//#define DEBUG_MSG
const char ENERGY_A_POS_DI_1997[]  = {0x90,0x10};  //正向有功总电量
const char ENERGY_A_NEG_DI_1997[] = {0x90,0x20}; 
const char  ENERGY_R_POS_DI_1997[] = {0x91,0x10}; 
const char  ENERGY_R_NEG_DI_1997[] = {0x91,0x20}; 
const char  POWER_U_A_DI_1997[] = {0xB6,0x11}; 
const char  POWER_U_B_DI_1997[] = {0xB6,0x12}; 
const char  POWER_U_C_DI_1997[] = {0xB6,0x13}; 
const char  POWER_I_A_DI_1997[] = {0xB6,0x21}; 
const char  POWER_I_B_DI_1997[] = {0xB6,0x22}; 
const char  POWER_I_C_DI_1997[] = {0xB6,0x23}; 
const char  POWER_P_DI_1997[] = {0xB6,0x30}; 
const char  POWER_Q_DI_1997[] = {0xB6,0x40}; 
const char  POWER_FACTOR_DI_1997[]={0xB6,0x50}; 
const char  POWER_FRQ_DI_1997[]={0xB6,0x64}; 

static int tim_read_645_97(int fd,int timeout,unsigned char * rcv_buf);
static int tim_packet_check_645_97(unsigned char * rcv_buf,unsigned char * meter_add,unsigned char * di,int rcv_num);
...
{  
       unsigned char snd_buf[100] = {0xfe,0xfe,0xfe,0x68,0x00,0x00,0x00,0x00,0x00,0x00,0x68,0x01,0x02,0x00,0x00,0x00,0x16};
	...
	strcpy(meter_add_temp,data->address);
	...
	for(i = 0 ; i < data->channel_num ; i++)
	{
		...
		{
			case ENERGY_A_POS_T:
				memcpy(temp_di, ENERGY_A_POS_DI_1997,2);
				di_code = 1;
				break;
			case ENERGY_A_NEG_T:
				memcpy(temp_di, ENERGY_A_NEG_DI_1997,2);
				di_code = 2;
				break;
			case ENERGY_R_POS_T:
				memcpy(temp_di, ENERGY_R_POS_DI_1997,2);
				di_code = 3;
				break;
			case ENERGY_R_NEG_T:
				memcpy(temp_di, ENERGY_R_NEG_DI_1997,2);
				di_code = 4;
				break;
			case POWER_U_A:
				memcpy(temp_di, POWER_U_A_DI_1997,2);
				di_code = 5;
				break;
			case POWER_U_B:
				memcpy(temp_di, POWER_U_B_DI_1997,2);
				di_code = 6;
				break;
			case POWER_U_C:
				memcpy(temp_di, POWER_U_C_DI_1997,2);
				di_code = 7;
				break;
			case POWER_I_A:
				memcpy(temp_di, POWER_I_A_DI_1997,2);
				di_code = 8;
				break;
			case POWER_I_B:
				memcpy(temp_di, POWER_I_B_DI_1997,2);
				di_code = 9;
				break;
			case POWER_I_C:
				memcpy(temp_di, POWER_I_C_DI_1997,2);
				di_code = 10;
				break;
			case POWER_P_T:
				memcpy(temp_di, POWER_P_DI_1997,2);
				di_code = 11;
				break;
			case POWER_Q_T:
				memcpy(temp_di, POWER_Q_DI_1997,2);
				di_code = 12;
				break;
			case POWER_FACTOR:
				memcpy(temp_di, POWER_FACTOR_DI_1997,2);
				di_code = 13;
				break;
			case POWER_FEQ:
				memcpy(temp_di, POWER_FRQ_DI_1997,2);
				di_code = 14;
				break;
			default:
				di_code = 0;
		...
		for(j = 0 ; j < 2; j ++)
		{
			snd_buf[13 + j] = temp_di[1 - j] + 0x33;
		}
		snd_buf[15] = 0;
		for(j = 3 ; j < 15; j++)
		{
			snd_buf[15] += snd_buf[j];
		}
#ifdef DEBUG
		...
		ret = write(data->fd, snd_buf, 17); 
		ret = tim_read_645_97(data->fd,2000,rcv_buf);
		if(ret > 0)
		{
			...
			ck_ret = tim_packet_check_645_97(rcv_buf,snd_buf + 4,snd_buf + 13,  ret);
			if(ck_ret == 0 )
			{
				switch(di_code)
				{
					...
					case 4:
						...
						{
							data->pchannel_data[i].value = tim_get_e_data(rcv_buf + 12);
					...
					case 7:
						...
						{
							data->pchannel_data[i].value = tim_get_u_data(rcv_buf + 12);
							data->pchannel_data[i].status = 1;
						}
						break;
					case 8:
					...
					case 10:
						...
						{
							data->pchannel_data[i].value = tim_get_i_data(rcv_buf + 12);
							data->pchannel_data[i].status = 1;
						}
						break;
					case 11:
						...
						{
							data->pchannel_data[i].value = tim_get_p_data(rcv_buf + 12);
							data->pchannel_data[i].status = 1;
						}
						break;
					case 12:
					...
					case 13:
						...
						{
							data->pchannel_data[i].value = tim_get_f_data(rcv_buf + 12);
							data->pchannel_data[i].status = 1;
						}
						break;
					case 14:
						...
						{
							data->pchannel_data[i].value = tim_get_frq_data(rcv_buf + 12);
							data->pchannel_data[i].status = 1;
						}
						break;
					default:
...
static int tim_packet_check_645_97(unsigned char * rcv_buf,unsigned char * meter_add,unsigned char * di,int rcv_num)
{
       ...
       if(rcv_buf[9]  < 2)
	{
		return -5;
      	...
      	if((di[0] != rcv_buf[10]) || (di[1] != rcv_buf[11]))
	{
		return -6;
	}   
	else
